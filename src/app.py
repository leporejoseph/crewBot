...
TOOLS = [
    ...
    {"name": "SerperDevTool", "needsApiKey": False, "source": "crewai", "description": "A specialized tool for development purposes, with specific functionalities under development."},
...
def create_new_agent_form():
    ...
    with agent_form.form(key="agent_form", clear_on_submit=True):
        ...
        tools = st.multiselect("Tools", TOOL_NAMES, key="tools_input")
...
def create_new_task_form():
    ...
    with task_form.form(key="task_form", clear_on_submit=True):
        ...
        tools = st.multiselect("Tools", TOOL_NAMES, key="task_tools_input")
...
def create_new_crew_container():
    with st.container(border=True):
        tab1, tab2, tab3 = st.tabs(["Agents", "Tasks", "Crew"])
        ...
        with tab1:
            ...
            update_agent_list(agent_list_container)
            if st.button("Next", key="next_to_tasks"):
                st.query_params.tab = "Tasks"
        ...
        with tab2:
            ...
            if not st.session_state.new_agents:
                ...
                update_task_list(task_list_container)
                if st.button("Next", key="next_to_crew"):
                    st.query_params.tab = "Crew"
                ...
                with st.chat_message("assistant"):
                    with st.spinner("Agents Working..."):
                        with st.expander("Crew Interaction Logs", False):
                            ...
                            response = dynamic_crew_handler.create_crew()
                    st.empty().text(f"Total Time Elapsed: {time.time() - start_time:.2f} seconds")
                    st.header(f"Tasks for {st.session_state.crew_list[i]['name']}:")
                    st.table({"Tasks": task_values})
                    st.header("Results:")
                    st.write(response)
                    st.session_state.messages.append({"role": "assistant", "content": response})

# Display create new crew form if required
